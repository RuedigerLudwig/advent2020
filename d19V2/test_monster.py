...
class TestMonsterRegExMatcher(unittest.TestCase):
    def setUp(self):
        self.lines = read_file("d19/data/sample1.txt")
        pass
...
class TestMingledMonsterRegexMatcher(unittest.TestCase):
    def setUp(self):
    ...
    def test_check_orig_match3(self):
        expected = False
        rules, _ = monster.parse_lines(self.lines)
        result = rules[0].matches("babaaabbbaaabaababbaabababaaab", rules)
        self.assertEqual(expected, result)
